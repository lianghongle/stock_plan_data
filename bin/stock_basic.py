...
def stock_basics(date=None, cache_file=False, debug=False):
    ...
    # 要存储股票信息的表名
    table_stock_basic = conf_talbes.STOCK_BASIC
    ...
    if check_table_result != None:
        check_table_data_sql = "SELECT * FROM {} where `code` = '000001' and `created_date` = '{}'".format(table_stock_basic,
                                                                                                date)
    ...
    if check is None:
        ...
        all_stock.to_sql(table_stock_basic, engine, if_exists='append', index=False) ## replace/append
    else:
        print(date + '数据已经存在')
